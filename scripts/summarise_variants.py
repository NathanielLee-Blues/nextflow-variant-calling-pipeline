...
if len(sys.argv) != 3:
    raise SystemExit("Usage: summarise_variants.py <input.vcf> <output_dir>")

vcf_path = Path(sys.argv[1])
outdir = Path(sys.argv[2])
outdir.mkdir(parents=True, exist_ok=True)

summary_csv = outdir / "called_variants_summary.csv"
counts_csv = outdir / "variant_type_counts.csv"
...
with summary_csv.open("w", newline="") as handle:
    writer = csv.DictWriter(
        handle,
        fieldnames=["chromosome", "position", "reference", "alternate", "quality", "filter", "depth", "variant_type"]
    )
    writer.writeheader()
